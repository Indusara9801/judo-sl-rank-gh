...
import { loadingStateConst, localStorageKeys, url } from "../../constants";
...
export const signUpUser = createAsyncThunk(
  ...
  async (userData, { dispatch }) => {
    ...
    try {
      await axios.post(`${url}/auth/account`, userData);
      dispatch(
        loadingStateActions.setLoadingState({
...
export const logInUser = createAsyncThunk(
  ...
  async (userData, { dispatch }) => {
    ...
    try {
      const response = await axios.post(`${url}/auth/authenticate`, userData);
      ...
      localStorage.setItem(
        localStorageKeys.TYPE,
        formatRole(response.data.status)
      );
      return {
        user: userData.email,
